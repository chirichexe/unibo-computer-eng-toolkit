...
table nat {
    ...
    # struttura fissa per nat

    # qui si cambia la destinazione
    chain PREROUTING {
        # le type sono fatte così standard per il NAT (vedi sopra opzioni usate se serve)
        type nat hook prerouting priority dstnat
        # quando arriva un pacchetto del genere,
        # cambia l'indirizzo di destinazione a 10.3.3.1 (il source rimane lo stesso).
        iif eth1 ip saddr 10.2.2.1 ip daddr 10.2.2.254 icmp type echo-request dnat to 10.3.3.1
    }
    
    # qui si cambia il source.
    # si modifica a posteriori per permette di analizzare l'indirizzo sorgente durante la forward
    chain POSTROUTING {
        # ''
        type nat hook postrouting priority srcnat
        # 
        oif eth3 ip saddr 10.2.2.1 ip daddr 10.3.3.1 icmp type echo-reply snat to 10.3.3.254
    }
...
table ip filter {
    ...
    chain FORWARD {
        ...
        ip saddr 10.2.2.1 ip daddr 10.3.3.1 iif eth1 oif eth3 icmp type echo-request accept
        ip saddr 10.2.2.1 ip daddr 10.3.3.1 oif eth1 iif eth3 icmp type echo-reply ct state established accept
    }

    chain INPUT {
        type filter hook forward priority filter; policy drop;
        ip daddr 10.1.1.1 ip saddr 10.3.3.1 iif eth1 tcp sport 22 ct state established accept
    ...
    chain OUTPUT {
        ...
        ip saddr 10.1.1.1 ip daddr 10.3.3.1 oif eth1 tcp dport 22 accept
