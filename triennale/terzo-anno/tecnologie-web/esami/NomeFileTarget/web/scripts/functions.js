...
function checkText(word) {
    ...
    // Controlla se la stringa è alfabetica
    let isAlphabetic = true;

    for (let char of word) {
        if ( (char < 'A' || char > 'Z') && (char < 'a' || char > 'z') && (char !== '.') && (char !== symbol) ) {
			// includo anche il punto per l'estensione
            isAlphabetic = false;
        }
    }

    if (!isAlphabetic) {
        console.error("La stringa deve essere solo alfabetica.");
        return false;
    }
...
input.addEventListener('input', () => {
    ...
    if (word.includes(symbol)) {
        ...
        // Esegue il controllo
        if (checkText(word) && checkText(target.value)) {
            // Invio il form
            inputForm.submit();
        }
    }
...
function sendData(tipo, testo){
	...
	.then(data => {
	    ...
	    if (data.error){
			...
			resultDiv.innerText = "Risposta sconosciuta";
		}
	})
	.catch(error => {
