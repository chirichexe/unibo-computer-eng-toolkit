    ...
    reader.onload = async function (event) {
        try {
            ...
            if (!validateMatrix(matrix1) || !validateMatrix(matrix2)) {
                resultDiv.innerHTML = "Errore: Le matrici contengono valori nulli o non numerici.";
                return;
            }
...
// Funzione per validare una matrice
function validateMatrix(matrix) {
    if (!Array.isArray(matrix) || matrix.length === 0) return false;

    for (const row of matrix) {
        if (!Array.isArray(row) || row.some(element => element === null || typeof element !== 'number')) {
            return false;
        }
    }
